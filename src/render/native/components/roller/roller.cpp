...
void Roller::setOptions (std::vector<std::string>& options, uint32_t mode) {
    if (options.size() == 0) {
        lv_roller_set_options(this->instance, "\0", static_cast<lv_roller_mode_t>(mode));
    } else {
        std::string str;
        ...
        lv_roller_set_options(this->instance, str.c_str(), static_cast<lv_roller_mode_t>(mode));
    }
};
